...
#' @title God mode
#' @description Add ROI columns to variables table
#' @param vgrid A data frame created by the \code{\link{variable_grid}} function.
#' @return The variable grid with the new ROI columns.
#' @examples variables <- enable_god_mode(variables)
#' @export
enable_god_mode <- function(vgrid){
  if(is.data.frame(vgrid)){
    vgrid$spend_variable <- NA
    vgrid$roi <- NA
    vgrid$confidence <- 0.01
  }
  else message("Please supply a data frame of model variables.")
  return(vgrid)
}
...
#' @export
god_mode <- function(data, y, variables,
                       observations=NULL,
                       cores=4,
                       chains=4,
                       adstock_range=c(0.1, 0.7),
                       chainlength=2000){
  message("Use this wisely. With strong priors comes great responsibility.")
  # Error handling ----
  # Missing date field
  if(!("date" %in% names(data))){
    stop("date column not found in data. Please supply a column with name 'date' containing a list of observation labels.")
  ...
  # God mode ----
  if("roi" %in% names(variables) && sum(is.na(variables$roi))!=nrow(variables)){
    god_mode_vars <- variables %>% filter(
      !is.na(roi)
    )
    if(NA %in% god_mode_vars$spend_variable){
      stop("Please provide a spend variable along with roi when using god mode.")
    }
    for(i in 1:nrow(god_mode_vars)){
      roi <- god_mode_vars[i,]$roi
      var <- god_mode_vars[i,]$variable
      spend_var <- god_mode_vars[i,]$spend_variable
      cost_per <- 
        sum(data[[spend_var]][data$date %in% observations]) / 
        sum(data[[var]][data$date %in% observations])
      if(tolower(god_mode_vars[i,]$transformation) == "media"){
        target_val <- max(data[[spend_var]][data$date %in% observations]) *
          0.8
        variables[variables$variable==var,]$prior.mean <- 
          variables[variables$variable==var,]$roi * 
          cost_per *
          target_val
        variables[variables$variable==var,]$prior.sd <- 
          variables[variables$variable==var,]$confidence * 
          cost_per *
          target_val
      }
      else{
        variables[variables$variable==var,]$prior.mean <- 
          variables[variables$variable==var,]$roi * cost_per
        variables[variables$variable==var,]$prior.sd <- 
          variables[variables$variable==var,]$confidence * cost_per
      }
    }
    ...
    seed_model <- bayesmodel(data, y, variables, observations,
                             cores, chains, adstock_range, chainlength)
    ...
    for(i in 1:nrow(god_mode_vars)){
      var <- god_mode_vars[i,]$variable
      roi <- god_mode_vars[i,]$roi
      spend <- god_mode_vars[i,]$spend_variable
      cost_per <- 
        sum(data[[spend_var]][data$date %in% observations]) / 
        sum(data[[var]][data$date %in% observations])
      contribution <- seed_model$decomp %>% 
        filter(date %in% observations &
                 variable == var) %>% 
        .$value %>% sum
      print(c("Contribution", contribution))
      target <- sum(data[[spend]][data$date %in% observations]) *
        roi
      print(c("Target", target))
      ratio <- target / contribution
      print(c("Ratio", ratio))
      
      variables$prior.mean[variables$variable==var] <- 
        variables$prior.mean[variables$variable==var] * ratio
      variables$prior.sd[variables$variable==var] <- 
        variables$prior.sd[variables$variable==var] * ratio
    }
  }
  
  message("Optimised priors:")
  variables %>% glimpse
